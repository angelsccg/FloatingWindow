...
public class StepService extends Service implements SensorEventListener {
    ...
    private class MessengerHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case KeyConfig.MSG_FROM_CLIENT:
                    try {
                        //这里负责将当前的步数发送出去，可以在界面或者其他地方获取，我这里是在MainActivity中获取来更新界面
                        Messenger messenger = msg.replyTo;
                        Message replyMsg = Message.obtain(null, KeyConfig.MSG_FROM_SERVER);
...
//        CURRENT_DATE = TimeUtil.getCurrentDate();
        CURRENT_DATE = DateUtil.longToString(System.currentTimeMillis(),"yyyy年MM月dd日 HH时mm分");
        //获取当天的数据，用于展示
        StepEntity stepEntity = StepDBManager.query(System.currentTimeMillis());
//        StepEntity entity = stepDataDao.getCurDataByDate(CURRENT_DATE);
        //为空则说明还没有该天的数据，有则说明已经开始当天的计步了
        if (stepEntity == null) {
            CURRENT_STEP = 0;
        } else {
            CURRENT_STEP = stepEntity.getStep();
        }
    }
    ...
    private void isNewDay() {
        String time = "00:00";
        if (time.equals(new SimpleDateFormat("HH:mm").format(new Date())) || !CURRENT_DATE.equals(DateUtil.longToString(System.currentTimeMillis(),"yyyy年MM月dd日 HH时mm分"))) {
            initTodayData();
        }
    }
    ...
    private void saveStepData() {
        StepDBManager.save(CURRENT_STEP);

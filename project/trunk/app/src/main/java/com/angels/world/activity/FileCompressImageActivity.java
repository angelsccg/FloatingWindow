 ...
 * 压缩图片
 * */
public class FileCompressImageActivity extends BaseActivity implements View.OnClickListener{

    int REQUESTCODE_FROM_ACTIVITY_01 = 1000;
    private Button btnSelect, btnCompress;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_file_compress_image);
...
//        String fileName = etFileName.getText().toString();
        if(pathStr == null || TextUtils.isEmpty(pathStr)){
            AcToastUtil.showShort(this,"文件路径有误");
            return;
        }
        File pathFile = new File(pathStr);
        File[] files = pathFile.listFiles();
        for (int i = 0; i < files.length; i++) {
            AcFileUtil.compressImage(files[i].getAbsolutePath(),500,300);
        }

...
public class RabbitView extends ImageView {
    ...
    public RabbitView(Context context, WindowManager.LayoutParams wmParams) {
        super(context);
        init();
...
//        initRightWalk();
    ...
    private void walkToRight(){
        if(x >= WorldService.screenWeight - getWidth()){
            walkToRandom();
        }else {
            updateViewPosition(x+=speed,y);
        }
    ...
    private void walkToDowm(){
        if(y <= WorldService.screenHeight - getHeight()){
            walkToRandom();
        }else {
